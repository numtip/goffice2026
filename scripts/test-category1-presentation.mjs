...
describe('Phase F — cross-link journeys and view-model', () => {
  ...
  it('1.1 presentation is wired with scope, policy, targets, and shared plan journeys', () => {
    ...
    assert.match(policy, /buildPolicyApproval2569/);
    assert.match(targets, /data-cat11-targets/);
    assert.match(targets, /targetPercent|baselineYear/);
    assert.match(targets, /metric famil/i);
    assert.match(targets, /buildTargetBoard2569/);
    assert.match(plan, /data-cat11-plan/);
    assert.match(plan, /proj-2569-plan-1|buildAnnualPlanView2569/);
    ...
    const fyvm = readFileSync(join(ROOT, 'src/utils/category1-fy2569-presentation.ts'), 'utf8');
    assert.match(fyvm, /buildScopeView2569/);
    assert.match(fyvm, /buildPolicyApproval2569/);
    assert.match(fyvm, /buildAnnualPlanView2569/);
  });
  ...
  it('1.2 presentation is wired with governance journey and truthful 1.2.2 gap', () => {
    ...
    assert.match(gov, /Historical Baseline|historical baseline/);
    assert.match(gov, /buildOrganizationCoverage2569|buildOrganizationCoverage/);
    assert.match(gov, /personnelCoverageTotal|orgSum|97/);
    assert.match(gov, /\/about\/committee\//);
    assert.match(gov, /focus-visible:ring-2/);
    assert.match(gov, /buildAppointmentAuthority2569/);
